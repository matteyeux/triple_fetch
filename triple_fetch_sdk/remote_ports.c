...
remote_set_exception_ports(mach_port_t task_port,
  ...
  mach_port_name_t remote_exception_port_name = push_local_port(task_port,
                                                                new_port,
                                                                MACH_MSG_TYPE_COPY_SEND);

  mach_port_name_t remote_task_self = (mach_port_name_t) call_remote(task_port, task_self_trap, 0);
      
  kern_return_t err = (kern_return_t) call_remote(task_port, task_set_exception_ports, 5,
                                                  REMOTE_LITERAL(remote_task_self),
                                                  REMOTE_LITERAL(exception_mask),
                                                  REMOTE_LITERAL(remote_exception_port_name), // send the port
                                                  REMOTE_LITERAL(behavior), // we want to receive a catch_exception_raise message with the thread port for the crashing thread
                                                  REMOTE_LITERAL(new_flavor));
  ...
  // should also drop refs on those ports we pushed (and self in the remote process)
